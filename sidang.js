// sidang.js (LENGKAP dengan Performance Logging)
...
Pertanyaan: ${question}`;
  ...
  try {
    ...
    console.log(`⏱️ [PERF] Gemini Inference Time: ${duration} seconds`);
...
LIMIT 1`;
    ...
    // [TIMER END] Opsional: Log waktu pencarian karyawan
    // const endDB = performance.now();
    // console.log(`⏱️ [PERF] Employee Lookup DB Time: ${((endDB - startDB)/1000).toFixed(4)}s`);
...
async function sendWhatsAppMessage(client, phone, message, typeLabel = 'General') {
  ...
  try {
    ...
    console.log(`⏱️ [PERF] WA Send Time (${typeLabel}) to ${phone}: ${duration} seconds`);
...
async function getMonthlyAttendance(nik, month, year) {
  ...
  try {
    ...
    console.log(`⏱️ [PERF] DB Query Absensi: ${((end - start)/1000).toFixed(4)}s`);
...
async function getMonthlyLeave(nik, month, year) {
  ...
  try {
    ...
    const end = performance.now(); // [TIMER END]
    console.log(`⏱️ [PERF] DB Query Cuti: ${((end - start)/1000).toFixed(4)}s`);
...
async function getMonthlyIzin(nik, month, year) {
  ...
  try {
    ...
    const end = performance.now(); // [TIMER END]
    console.log(`⏱️ [PERF] DB Query Izin/Sakit: ${((end - start)/1000).toFixed(4)}s`);
...
AND p.jam_in >= DATE_SUB(NOW(), INTERVAL 10 MINUTE)`,
    ...
    if(rows.length > 0) console.log(`⏱️ [PERF] Cron CheckIn DB Query: ${dbTime}s found ${rows.length} rows`);
...
AND k.no_hp IS NOT NULL AND k.no_hp != ''`,
    ...
    // 2. Log Waktu Query DB
    const dbTime = ((performance.now() - jobStart) / 1000).toFixed(4);
    if(rows.length > 0) console.log(`⏱️ [PERF] Cron CheckOut DB Query took ${dbTime}s`);
...
AND k.no_hp IS NOT NULL AND k.no_hp != ''`,
    ...
    // 2. Log Waktu Query DB
    const dbTime = ((performance.now() - jobStart) / 1000).toFixed(4);
    if(rows.length > 0) console.log(`⏱️ [PERF] Cron Morning Reminder DB Query took ${dbTime}s`);
...
AND k.no_hp IS NOT NULL AND k.no_hp != ''`,
    ...
    // 2. Log Waktu Query DB
    const dbTime = ((performance.now() - jobStart) / 1000).toFixed(4);
    if(rows.length > 0) console.log(`⏱️ [PERF] Cron Afternoon Reminder DB Query took ${dbTime}s`);
...
client.on('message', async (msg) => {
  ...
  try {
    ...
    if (!employee) {
      ...
      console.log(`⏱️ [PERF] Total End-to-End Response Time (Non-Employee): ${totalDuration} seconds`);
      console.log(`-------------------------------------------`);
      return;
    }
    ...
    if (!greeted) {
      ...
      const msgEnd = performance.now();
      const totalDuration = ((msgEnd - msgStart) / 1000).toFixed(4);
      console.log(`⏱️ [PERF] Total End-to-End Response Time (Greeting): ${totalDuration} seconds`);
      console.log(`-------------------------------------------`);
      return; 
    }
    ...
    if (text === '/infoabsensi_bulanini') {
        ...
        const msgEnd = performance.now();
        console.log(`⏱️ [PERF] Total End-to-End Response Time (Command Absensi): ${((msgEnd - msgStart) / 1000).toFixed(4)} seconds`);
        console.log(`-------------------------------------------`);
        return;
    }
    ...
    if (text === '/infosakit_cutibulanini') {
        ...
        const msgEnd = performance.now();
        console.log(`⏱️ [PERF] Total End-to-End Response Time (Command Izin/Sakit): ${((msgEnd - msgStart) / 1000).toFixed(4)} seconds`);
        console.log(`-------------------------------------------`);
        return;
    }
    ...
    if (text && text.length > 1) {
      ...
      const msgEnd = performance.now();
      console.log(`⏱️ [PERF] Total End-to-End Response Time (AI Chat): ${((msgEnd - msgStart) / 1000).toFixed(4)} seconds`);
      console.log(`-------------------------------------------`);
      return;
    } else {
      ...
      const msgEnd = performance.now();
      console.log(`⏱️ [PERF] Total End-to-End Response Time (Default): ${((msgEnd - msgStart) / 1000).toFixed(4)} seconds`);
      console.log(`-------------------------------------------`);
      return;
    }
