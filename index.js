...
          // FIX: Ganti spasi ' ' dengan 'T' agar menjadi format ISO (YYYY-MM-DDTHH:MM:SS) yang valid untuk new Date()
          const jamIn = record.jam_in ? new Date(String(record.jam_in).replace(' ', 'T')).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
          const jamOut = record.jam_out ? new Date(String(record.jam_out).replace(' ', 'T')).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
...
S   // Reminder pagi (07:00) Senin-Jumat
